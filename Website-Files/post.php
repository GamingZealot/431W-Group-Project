...
if (isset($_POST['Submit1'])) {
	...
	if (empty($movie) || empty($price)|| empty($format) || empty($location) || empty($desc) || empty($method)){
		...
		$query = "INSERT INTO Movies (title, year, pictureURL, synopsys) VALUES ( '".$movie."','".$year."','".$photo."','".$synop.")";
		mysqli_query($db, $query) or die('Error querying database inserting user.');

		$query = "INSERT INTO Items (format, location, desciption) VALUES ( '".$format."','".$location."','".$desc.")";
		mysqli_query($db, $query) or die('Error querying database inserting user.');
		...
		$query = "SELECT I.itemId
					FROM Items as I
					WHERE I.format = '".$format."' AND I.location = '".$location."' AND I.desciption = '".$desc."'";
		mysqli_query($db, $query) or die('Error querying database inserting user.');
		$result = mysqli_query($db, $query);
		$row = mysqli_fetch_array($result);
		$itemID  = $row['itemId'];
		...
		$query = "SELECT Is_Seller.sellerId
					FROM  as ISe
		...
		if($method = "auction"){
			$query = "INSERT INTO AuctionItems (endTime, currentBid, reservePrice, itemId) VALUES ( '".$auctionEnd."','0', '".$price."','".$itemID."')";
			mysqli_query($db, $query) or die('Error querying database inserting user.');
		}
		if($method = "rent"){
...
<br>
<br>If Auction Was Selected, tell us...<br> End Time*:<br>
