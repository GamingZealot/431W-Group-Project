...
while ($row = mysqli_fetch_array($result)) {
 ...
 echo '<br><label style="margin=0">' . $row['year'] . '<br>';
 echo '' . $row['synopsis'] . '</label><br>';
 echo '<label id="' . $row['itemId'] . 's">Sales Price=</label><br>';
 echo '<label id="' . $row['itemId'] . 'r">Rental Price=</label><br>';
 echo '<label id="' . $row['itemId'] . 'u">Current Bid=</label><br>';
}

$query = "SELECT S.price, I.itemId
 FROM Categories C, Movies M, Items I, Categorized_as A, Is_Movie B, SaleItems S
 WHERE S.itemId=I.itemId AND C.categoryName = '".$cat."' AND C.categoryId = A.categoryId AND A.itemId = I.itemId AND I.itemId = B.itemId AND B.movieId = M.movieId AND M.title LIKE '%".$sch."%'";
mysqli_query($db, $query) or die('Error querying database.');

$result = mysqli_query($db, $query);

while ($row = mysqli_fetch_array($result)) {
	echo '<script type="text/javascript">document.getElementById("' . $row['itemId'] . 's").innerHTML = "Sales Price=' . $row['price'] . '"</script>';
}

$query = "SELECT R.rentPrice, I.itemId
 FROM Categories C, Movies M, Items I, Categorized_as A, Is_Movie B, RentableItems R
 WHERE R.itemId=I.itemId AND C.categoryName = '".$cat."' AND C.categoryId = A.categoryId AND A.itemId = I.itemId AND I.itemId = B.itemId AND B.movieId = M.movieId AND M.title LIKE '%".$sch."%'";
mysqli_query($db, $query) or die('Error querying database.');

$result = mysqli_query($db, $query);

while ($row = mysqli_fetch_array($result)) {
	echo '<script type="text/javascript">document.getElementById("' . $row['itemId'] . 'r").innerHTML = "Rental Price=' . $row['rentPrice'] . '"</script>';
}

$query = "SELECT U.currentBid, I.itemId
 FROM Categories C, Movies M, Items I, Categorized_as A, Is_Movie B, AuctionItems U
 WHERE U.itemId=I.itemId AND C.categoryName = '".$cat."' AND C.categoryId = A.categoryId AND A.itemId = I.itemId AND I.itemId = B.itemId AND B.movieId = M.movieId AND M.title LIKE '%".$sch."%'";
mysqli_query($db, $query) or die('Error querying database.');

$result = mysqli_query($db, $query);

while ($row = mysqli_fetch_array($result)) {
	echo '<script type="text/javascript">document.getElementById("' . $row['itemId'] . 'u").innerHTML = "Current Bid=' . $row['currentBid'] . '"</script>';
}
